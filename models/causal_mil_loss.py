...
def causal_mil_loss(model, bag_feats, attention_weights, topk_ratio=0.03, lam=0.5):
    ...
    y_orig, _ = model(bag_feats)
    soft_orig = F.softmax(y_orig, dim=1)
    ...
    replace_feats[:, low_idx, :] = bag_feats[:, topk_idx, :].clone()
