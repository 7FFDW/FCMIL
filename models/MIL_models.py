...
from models.modules import FAA
...
class DSMIL(nn.Module):
    ...
    def forward(self, x):
        ...
        return classes, logit, A, B,y_prob


class DyT_MIL(nn.Module):
    def __init__(self, D, init_a=1.0):
        super().__init__()
        self.a = nn.Parameter(torch.ones(1) * init_a)         # 全局缩放因子
        self.y = nn.Parameter(torch.ones(1, D))               # 每个维度一个缩放
        self.b = nn.Parameter(torch.zeros(1, D))              # 每个维度一个偏置

    def forward(self, x):
        # x: [N, D]
        x = torch.tanh(self.a * x)        # 全局激活调节
        return x * self.y + self.b
...
class ABMIL(nn.Module):
    def __init__(self, n_classes=2, act='relu', bias=False, dropout=False):
        ...
        # self.feature += [DyT_MIL_Enhanced(512)]
    ...
    def forward(self, x):
        ...
        return logits,Y_prob, Y_hat,A,x
...
class CLAM_SB(nn.Module):
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        return logits, Y_prob, Y_hat, A_raw,results_dict,h.mean(dim=0, keepdim=True)
...
class CLAM_MB(CLAM_SB):
    ...
    def forward(self, h, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        return logits, A_raw, results_dict, h.mean(dim=0, keepdim=True)
...
class FCMIL(nn.Module):
    ...
    def forward(self, x):
        ...
        h_freq, attn = self.freq_layer(h)
        z_total = h_spatial + h_freq
        z_bag = torch.mean(z_total, dim=1)
        logits = self.classifier(z_bag)


        return logits, attn.mean(dim=1)
