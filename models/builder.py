...
def has_UNI():
    HAS_UNI = False
    UNI_CKPT_PATH = r"E:\code\PMIL-main\wsi_core\uni_weights\pytorch_model.bin"
    # check if UNI_CKPT_PATH is set, catch exception if not
    # 2. 检查文件是否真的在那里
    if os.path.exists(UNI_CKPT_PATH):
        HAS_UNI = True
        print(f"✅ 找到 UNI 权重: {UNI_CKPT_PATH}")
    else:
        print(f"❌ 未找到 UNI 权重，请检查路径: {UNI_CKPT_PATH}")
